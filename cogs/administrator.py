...
class administrator(commands.Cog):
    ...
    @voice_create.command(
        name="리스트",
        description="[🔒 '서버 관리자' 전용] 서버에서 활성화된 '음챗 생성기' 목록을 확인해요.",
        checks=[is_server_enabled],
    )
    ...
    async def voice_create_list(self, ctx):
        ...
        embed = Embed.default(
            title=f"📃 {ctx.guild.name} 음성 생성기 목록",
            description=f"현재 총 ``{len(data)}개``의 음성 생성기가 등록되어 있어요.",
        )
        Embed.user_footer(embed, ctx.author)
        for voice in data:
            ...
            if voice_channel:
                embed.add_field(
                    ...
                    value=f">>> 생성 채널 : {voice_channel.mention}\n생성될 채널 이름 : ``{data[voice]['name']}``",
                    inline=False,
                )
            else:
                embed.add_field(
                    name="조회 불가",
                    value=f"해당 채널은 조회할 수 없어요. 삭제 후 재등록이 필요해요.",
                    inline=False,
                )
